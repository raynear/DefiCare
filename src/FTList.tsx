...
// function FTStatusStr(status: number) {
//   if (status === 0) {
//     return "OffSale";
//   } else if (status === 1) {
//     return "OnSale";
...
function FTList(props: any) {
  const classes = useStyles();

  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [filter, setFilter] = useState({ type: "FT", OffSale: false, OnSale: false, Rented: false, Sold: false })
  const [displayRows, setDisplayRows] = useState<IFT[]>([]);
  ...
  useEffect(() => {
    ...
    const web3 = new Web3((window as any).web3.currentProvider);
    const contract = new web3.eth.Contract(FTABI as any, FTContractAddress);
  ...
  // async function getAllFT(account: string, contract: any) {
  ...
  const handleClick = (e: React.MouseEvent<unknown>, name: string) => {
    props.history.push("/DefiCare/FT/" + name.toString());
  }
  ...
  function filtering(obj: any) {
    ...
    if (filter.OffSale && parseInt(obj.Status, 10) === FTStatus.OffSale) {
      return true;
    } else if (filter.OnSale && parseInt(obj.Status, 10) === FTStatus.OnSale) {
      return true;
    } else if (filter.Rented && parseInt(obj.Status, 10) === FTStatus.Rented) {
      return true;
    } else if (filter.Sold && parseInt(obj.Status, 10) === FTStatus.Sold) {
      return true;
    }
    return false;
  }
